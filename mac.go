...
func Vaild(key []byte, uid, ctr, mac []byte, input string) bool {
	if len(key) != 16 {
		panic(errors.New("key len error"))
	}
	if len(uid) != 7 {
	...
	maciv = append(maciv, uid...)
	maciv = append(maciv, ctr[2], ctr[1], ctr[0])
	mackey := CMAC(key, maciv)
	macv := CMAC8(mackey, []byte(input))
	return bytes.Equal(macv, mac)
}
